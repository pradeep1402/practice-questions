...
// Input: ["London", "Paris", "Tokyo"], { "London": { population: 9000000 }, "Paris": { population: 2000000 }, "Tokyo": { population: 14000000 } }
// Output: ["London", "Tokyo"]
const findCitiesAbovePopulationThreshold = function (cities, lookup, populationThreshold) {
  return cities.filter(city => lookup[city] && lookup[city].population > populationThreshold);
};

// Find animals that belong to a specific species based on a lookup object.
// Input: ["Lion", "Tiger", "Elephant"], { "Lion": { species: "Panthera leo" }, "Tiger": { species: "Panthera tigris" }, "Elephant": { species: "Elephas maximus" } }
// Output: ["Lion", "Tiger"]
const findAnimalsBySpecies = function (animals, lookup, species) {
  return animals.filter(animal => lookup[animal] && lookup[animal].species === species);
};

// Find books that have a rating above a given threshold based on a lookup object.
// Input: ["Book A", "Book B", "Book C"], { "Book A": { rating: 4.7 }, "Book B": { rating: 3.5 }, "Book C": { rating: 4.2 } }
// Output: ["Book A", "Book C"]
const findBooksAboveRating = function (books, lookup, ratingThreshold) {
  return books.filter(book => lookup[book] && lookup[book].rating > ratingThreshold);
};

// Find employees who have a certain skill based on a lookup object.
// Input: ["Alice", "Bob", "Charlie"], { "Alice": { skills: ["JavaScript", "React"] }, "Bob": { skills: ["Python", "Django"] }, "Charlie": { skills: ["JavaScript", "Node.js"] } }
// Output: ["Alice", "Charlie"]
const findEmployeesBySkill = function (employees, lookup, skill) {
  return employees.filter(employee => lookup[employee] && lookup[employee].skills.includes(skill));
};

// Find products that are in stock based on a lookup object.
// Input: ["Laptop", "Phone", "Tablet"], { "Laptop": { stock: 10 }, "Phone": { stock: 0 }, "Tablet": { stock: 5 } }
// Output: ["Laptop", "Tablet"]
const findInStockProducts = function (products, lookup) {
  return products.filter(product => lookup[product] && lookup[product].stock > 0);
};

// Find students who are enrolled in a specific course based on a lookup object.
// Input: ["John", "Alice", "Bob"], { "John": { courses: ["Math", "Science"] }, "Alice": { courses: ["Math"] }, "Bob": { courses: ["Science"] } }
// Output: ["John", "Alice"]
const findStudentsByCourse = function (students, lookup, course) {
  return students.filter(student => lookup[student] && lookup[student].courses.includes(course));
};

// Find countries that have a specific continent based on a lookup object.
// Input: ["India", "USA", "France"], { "India": { continent: "Asia" }, "USA": { continent: "North America" }, "France": { continent: "Europe" } }
// Output: ["India", "USA"]
const findCountriesByContinent = function (countries, lookup, continent) {
  return countries.filter(country => lookup[country] && lookup[country].continent === continent);
};
